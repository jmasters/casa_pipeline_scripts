...
def parse_command_line(argv):
    ...
    for idx, arg in enumerate(argv):
        if 'create_pipeline_task.py' in arg:
    ...
    try:
        ...
        print('Problem with create_pipeline_task arguments.')
        return
...
def create(area, task_name):
    try:
        repository_path = os.environ['SCIPIPE_HEURISTICS']
    except KeyError:
        print('Set SCIPIPE_HEURISTICS environment variable to pipeline repository directory and try again.')
    ...
    # -----------------------------------------------------------------------------
    module_template = Template(filename='{repo}/pipeline/infrastructure/create_pipeline_task/'
                                        'pipeline_task_module.mako'.format(repo=repository_path))
    cli_template = Template(filename='{repo}/pipeline/infrastructure/create_pipeline_task/'
                                     'pipeline_cli_module.mako'.format(repo=repository_path))
    cli_xml_template = Template(filename='{repo}/pipeline/infrastructure/create_pipeline_task/'
                                         'pipeline_cli_xml.mako'.format(repo=repository_path))
    init_template = Template(filename='{repo}/pipeline/infrastructure/create_pipeline_task/'
    ...
    # -----------------------------------------------------------------------------
    mako_template = '{repo}/pipeline/infrastructure/create_pipeline_task/pipeline_weblog_task.mako'.format(repo=repository_path)
